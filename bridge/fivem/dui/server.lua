...
function dui.createPoly4(target, options)
    return create("createPoly4", "pr_bridge:client:dui:createPoly4", target, options)
end

function dui.poly4(target, options)
    return dui.createPoly4(target, options)
end
...
function dui.setOpacity(target, id, opacity)
    if opacity == nil then
        opacity = id
        id = target
        target = -1
    ...
    if opacity == nil then return false, "invalid_opacity" end
    if id == nil then return false, "invalid_id" end

    local session = sessions[tostring(id)]
    if session and session.options then
        session.options.opacity = opacity
    end

    dispatch("pr_bridge:client:dui:setOpacity", target, tostring(id), opacity)
    return true
end

function dui.setBrightness(target, id, brightness)
    ...
    if brightness == nil then return false, "invalid_brightness" end
    if id == nil then return false, "invalid_id" end
    ...
    dispatch("pr_bridge:client:dui:setBrightness", target, tostring(id), brightness)
    return true
end
...
function dui.sync(target)
    ...
    for _, session in pairs(sessions) do
        ...
        elseif session.kind == "createPoly4" then
            dispatch("pr_bridge:client:dui:createPoly4", target, session.options)
        else
