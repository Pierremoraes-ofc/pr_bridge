...
local function getPolyScreenBounds(poly)
    if type(poly) ~= "table" or not poly.pointA or not poly.pointB then return nil end
...
local function drawDuiSprite(instance, options)
    ...
    local r, g, b, a = colorChannels(options.color)
...
local function drawDuiOnArea(instance, poly)
    local r, g, b, a = colorChannels(poly.color or { 255, 255, 255, 255 })
