...
local zonesCreated = {}
local fivemNet = PRCore and PRCore.load("@pr_bridge/bridge/fivem/net/client", _ENV)

local function getEntityFromNetId(netId)
    if fivemNet and fivemNet.getEntity then
        return fivemNet.getEntity(netId, 1000)
    end
...
local function getEntitiesFromNetIds(netIds)
    if type(netIds) == "number" then
        return getEntityFromNetId(netIds)
    end
    
    local entities = {}

    for i=1, #netIds do
        entities[i] = getEntityFromNetId(netIds[i])
    end
...
function target.addGlobalOption(options)
    if Debug then
        Debug("WARNING", "addGlobalOption not supported yet for qb-target")
    end
end

function target.removeGlobalOption(optionNames)
    if Debug then
        Debug("WARNING", "removeGlobalOption not supported yet for qb-target")
    end
end
