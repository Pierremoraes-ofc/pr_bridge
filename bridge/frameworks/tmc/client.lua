...
function framework.GetPlayerData() return framework.GetPlayer() end
...
function framework.GetPlayerJob() local info=framework.GetJobInfo(); if not info then return nil end; return {name=info.name,label=info.label,grade=info.grade,gradeLabel=info.gradeLabel} end
function framework.PlayerHasJob(jobName,grade) local job=framework.GetPlayerJob(); if not job or tostring(job.name or ""):lower()~=tostring(jobName or ""):lower() then return false end; if grade==nil then return true end; return (tonumber(job.grade) or 0)>=(tonumber(grade) or 0) end
